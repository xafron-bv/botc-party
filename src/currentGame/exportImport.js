...
function normalizeImportedGameState(data) {
  ...
  return {
    ...
    players,
    scriptMetaName:
      typeof state.scriptMetaName === 'string'
        ? state.scriptMetaName
        : typeof state.scriptName === 'string'
          ? state.scriptName
          : '',
    includeTravellers: !!state.includeTravellers,
    dayNightTracking: state.dayNightTracking || {
      enabled: false,
      phases: ['N1'],
      currentPhaseIndex: 0,
      reminderTimestamps: {}
...
export function exportCurrentGame({ grimoireState }) {
  const gameState = {
    ...
    scriptMetaName:
      typeof grimoireState.scriptMetaName === 'string' ? grimoireState.scriptMetaName : '',
...
export async function importCurrentGame({ file, grimoireState, grimoireHistoryList }) {
  ...
  if (Array.isArray(data)) {
    alert(
      'This appears to be a script file. Please use the "Upload Custom Script" option in the Game Setup section to load it.'
    );
    return;
  }
  if (isHistoryExportFile(data)) {
    alert(
      'This appears to be a user data history export file. Please use the "Import Data" button.'
    );
    return;
  }
  ...
  try {
    localStorage.setItem('botcAppStateV1', JSON.stringify(saved));
  } catch (_) {}
  try {
    localStorage.setItem(INCLUDE_TRAVELLERS_KEY, saved.includeTravellers ? '1' : '0');
  } catch (_) {}
  try {
    localStorage.setItem(MODE_STORAGE_KEY, saved.mode);
  } catch (_) {}
