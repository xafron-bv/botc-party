...
export function applyGrimoireHiddenState({ grimoireState }) {
  ...
  try {
    const banner = document.getElementById('grimoire-hidden-banner'); if (banner) banner.setAttribute('aria-hidden', String(!grimoireState.grimoireHidden));
  } catch (_) { }
  updateGrimoire({ grimoireState });
}
export const setGrimoireHidden = withStateSave(({ grimoireState, hidden }) => { grimoireState.grimoireHidden = !!hidden; applyGrimoireHiddenState({ grimoireState }); });
export function toggleGrimoireHidden({ grimoireState }) { setGrimoireHidden({ grimoireState, hidden: !grimoireState.grimoireHidden }); }
...
function mountBluffTokensContainer({ grimoireState }) {
  ...
  const bluffContainer = createBluffTokensContainer({ grimoireState }); const parent = document.getElementById('grimoire') || document.getElementById('center');
  if (parent) { parent.appendChild(bluffContainer); }
}
export const setupGrimoire = withStateSave(({ grimoireState, grimoireHistoryList, count }) => {
  ...
  requestAnimationFrame(() => { repositionPlayers({ grimoireState }); updateGrimoire({ grimoireState }); renderSetupInfo({ grimoireState }); });
});
export function updateGrimoire({ grimoireState }) {
  const abilityTooltip = document.getElementById('ability-tooltip'); const playerCircle = document.getElementById('player-circle');
  const listItems = playerCircle.querySelectorAll('li'); renderSetupInfo({ grimoireState });
  if (grimoireState.grimoireHidden && abilityTooltip) { abilityTooltip.classList.remove('show'); }
  const showNightReminders = shouldShowNightOrder(grimoireState); const nightOrderMap = showNightReminders ? calculateNightOrder(grimoireState) : {};
  const currentPhase = getCurrentPhase(grimoireState); const isFirstNight = currentPhase === 'N1';
  ...
  }); positionInfoIcons(); positionTokenReminders(); updateAllBluffTokens({ grimoireState });
}
export const resetGrimoire = withStateSave(({ grimoireState, grimoireHistoryList, playerCountInput, preserveBag = false }) => {
  ...
  try { document.body.classList.remove('grimoire-snapshot-active'); } catch (_) { }
  try {
    if (!grimoireState.playerSetup) { grimoireState.playerSetup = { bag: [], assignments: [], revealed: false }; } else {
      if (!preserveBag) grimoireState.playerSetup.bag = []; grimoireState.playerSetup.assignments = []; grimoireState.playerSetup.revealed = false;
  ...
  const newPlayers = Array.from({ length: playerCount }, (_, i) => {
    const existing = existingPlayers[i]; const name = existing && existing.name ? existing.name : `Player ${i + 1}`; return createEmptyPlayer(name);
  }); grimoireState.players = newPlayers; rebuildPlayerCircleUiPreserveState({ grimoireState }); grimoireState.bluffs = [null, null, null];
  try {
    ...
    hideDayNightSlider(); updateDayNightUI(grimoireState);
  } catch (_) { }
...
export const rebuildPlayerCircleUiPreserveState = withStateSave(({ grimoireState }) => {
  const playerCircle = document.getElementById('player-circle'); const playerCountInput = document.getElementById('player-count'); if (!playerCircle) return;
  ...
  }); mountBluffTokensContainer({ grimoireState }); repositionPlayers({ grimoireState }); updateGrimoire({ grimoireState }); renderSetupInfo({ grimoireState });
  requestAnimationFrame(() => { repositionPlayers({ grimoireState }); updateGrimoire({ grimoireState }); });
});
    ...
    document.addEventListener('touchstart', (e) => {
      ...
      if (!target.closest('#player-circle li')) {
        document.querySelectorAll('#player-circle li[data-raised="true"]').forEach(el => {
          delete el.dataset.raised; el.style.zIndex = el.dataset.originalLiZIndex || ''; delete el.dataset.originalLiZIndex;
        });
      }
    }, { passive: true });
  }
}); document.addEventListener('pointerdown', closeMenusOnOutsideEvent, true); document.addEventListener('touchstart', closeMenusOnOutsideEvent, true);
document.addEventListener('keydown', (e) => {
  const grimoireState = window.grimoireState; if (!grimoireState) return;
  if (e.key === 'Escape') {
    if (grimoireState.playerContextMenu && grimoireState.playerContextMenu.style.display === 'block') { hidePlayerContextMenu({ grimoireState }); }
    if (grimoireState.reminderContextMenu && grimoireState.reminderContextMenu.style.display === 'block') { hideReminderContextMenu({ grimoireState }); }
  }
});
