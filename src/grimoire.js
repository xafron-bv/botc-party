...
try {
  window.openReminderTokenModal = openReminderTokenModal;
} catch (_) {}
function setupPlayerNameHandlers({ listItem, grimoireState, playerIndex }) {
  ...
  if ('ontouchstart' in window) {
    attachTouchHandler(
      listItem.querySelector('.player-name'),
      (e) => {
...
export function showGrimoire({ grimoireState }) {
  setGrimoireHidden({ grimoireState, hidden: false });
}
...
export const takeGrimoireSnapshot = withStateSave(({ grimoireState }) => {
  ...
  try {
    document.body.classList.add('grimoire-snapshot-active');
  } catch (_) {}
});
export const restoreGrimoireSnapshot = withStateSave(({ grimoireState }) => {
  ...
  try {
    document.body.classList.remove('grimoire-snapshot-active');
  } catch (_) {}
...
export function applyGrimoireSnapshotState({ grimoireState }) {
  try {
    document.body.classList.toggle('grimoire-snapshot-active', hasGrimoireSnapshot(grimoireState));
  } catch (_) {}
}
...
export const setupGrimoire = withStateSave(({ grimoireState, grimoireHistoryList, count }) => {
  ...
  try {
    if (
      ...
      !grimoireState.isRestoringState &&
      Array.isArray(grimoireState.players) &&
  ...
  grimoireState.players = Array.from({ length: count }, (_, i) =>
    createEmptyPlayer(`Player ${i + 1}`)
  );
  if (playerCountInput) {
    try {
      playerCountInput.value = String(grimoireState.players.length);
    } catch (_) {}
  }
...
export const resetGrimoire = withStateSave(
  ({ grimoireState, grimoireHistoryList, playerCountInput, preserveBag = false }) => {
    ...
    const playerCount = playerCountInput
      ? parseInt(playerCountInput.value, 10)
      : (grimoireState.players || []).length;
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Escape') {
    if (
      grimoireState.playerContextMenu &&
      grimoireState.playerContextMenu.style.display === 'block'
    ) {
      hidePlayerContextMenu({ grimoireState });
    }
    if (
      grimoireState.reminderContextMenu &&
      grimoireState.reminderContextMenu.style.display === 'block'
    ) {
