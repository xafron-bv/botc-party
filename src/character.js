...
export function populateCharacterGrid({ grimoireState }) {
  ...
  const includeModalTravellers =
    document.getElementById('include-travellers-in-modal')?.checked || false;
...
export function rebuildAllRoles({ grimoireState }) {
  ...
  (grimoireState.players || []).forEach((player) => {
    ...
    if (!id) return;
    const role =
      grimoireState.extraTravellerRoles?.[id] || grimoireState.scriptTravellerRoles?.[id];
...
export function applyTravellerToggleAndRefresh({ grimoireState }) {
  if (Array.isArray(grimoireState.scriptData))
    displayScript({ data: grimoireState.scriptData, grimoireState }).catch(console.error);
}
    ...
    characterIds.forEach((characterItem) => {
      if (typeof characterItem === 'string' && characterItem !== '_meta') {
        ...
        if (canonicalId && roleLookup[canonicalId]) {
          ...
          console.log(
            `Resolved character ${characterItem} -> ${canonicalId} (${roleLookup[canonicalId].name})`
          );
        } else {
        ...
        if (canonicalId && roleLookup[canonicalId]) {
          ...
          console.log(
            `Resolved object character ${characterItem.id} -> ${canonicalId} (${roleLookup[canonicalId].name})`
          );
          ...
          if (characterItem.remindersGlobal)
            customRole.remindersGlobal = characterItem.remindersGlobal;
          ...
          if (typeof characterItem.firstNight === 'number')
            customRole.firstNight = characterItem.firstNight;
          if (typeof characterItem.otherNight === 'number')
            customRole.otherNight = characterItem.otherNight;
          if (typeof characterItem.firstNightReminder === 'string')
            customRole.firstNightReminder = characterItem.firstNightReminder;
          if (typeof characterItem.otherNightReminder === 'string')
            customRole.otherNightReminder = characterItem.otherNightReminder;
    ...
    characterIds.forEach((characterItem) => {
      if (typeof characterItem === 'string' && characterItem !== '_meta') {
        ...
        if (characterItem.name && characterItem.team && characterItem.ability) {
          ...
          if (typeof characterItem.firstNight === 'number')
            customFallback.firstNight = characterItem.firstNight;
          if (typeof characterItem.otherNight === 'number')
            customFallback.otherNight = characterItem.otherNight;
          if (typeof characterItem.firstNightReminder === 'string')
            customFallback.firstNightReminder = characterItem.firstNightReminder;
          if (typeof characterItem.otherNightReminder === 'string')
            customFallback.otherNightReminder = characterItem.otherNightReminder;
...
export function openCharacterModal({ grimoireState, playerIndex }) {
  if (!grimoireState.scriptData && grimoireState.scriptLoadPromise) {
  ...
  if (includeModalTravellersCheckbox) {
    ...
    includeModalTravellersCheckbox.removeEventListener(
      'change',
    ...
    includeModalTravellersCheckbox.addEventListener(
      'change',
      includeModalTravellersCheckbox._modalChangeHandler
    );
  }
...
export const onIncludeTravellersChange = withStateSave(
  ({ grimoireState, includeTravellersCheckbox }) => {
...
export const loadAllCharacters = withStateSave(async ({ grimoireState }) => {
  ...
  try {
    ...
    grimoireState.scriptData = [
      { id: '_meta', name: 'All Characters', author: 'System' },
      ...characterIds
    ];
    rebuildAllRoles({ grimoireState });
    applyTravellerToggleAndRefresh({ grimoireState });
