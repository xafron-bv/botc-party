...
export async function populateReminderTokenGrid({ grimoireState }) {
  ...
  const delegatedSelectionHandler = withStateSave((e) => {
    const tokenEl = e.target && (e.target.closest && e.target.closest('#reminder-token-grid .token')); if (!tokenEl) return;
    try { e.preventDefault(); } catch (_) { }
    try { e.stopPropagation(); } catch (_) { }
    if (!canOpenModal({ grimoireState })) return; const label = tokenEl.dataset.tokenLabel || ''; const id = tokenEl.dataset.tokenId || '';
  ...
  try {
    ...
    const filtered = reminderTokens.filter(t => {
      ...
      const terms = filter.split(/\s+/).filter(Boolean); return terms.every(term => combined.includes(term));
    });
    ...
    console.error(e); const msg = document.createElement('div'); msg.style.color = '#ccc'; msg.textContent = 'No reminder tokens available.'; reminderTokenGrid.appendChild(msg);
  }
}
export function openReminderTokenModal({ grimoireState, playerIndex }) {
  if (!canOpenModal({ grimoireState })) return; const reminderTokenModal = document.getElementById('reminder-token-modal');
  const reminderTokenSearch = document.getElementById('reminder-token-search'); const reminderTokenModalPlayerName = reminderTokenModal.querySelector('.modal-player-name');
  if (!reminderTokenModal) return; grimoireState.selectedPlayerIndex = playerIndex;
  if (reminderTokenModalPlayerName) reminderTokenModalPlayerName.textContent = grimoireState.players[playerIndex].name; if (reminderTokenSearch) reminderTokenSearch.value = '';
  reminderTokenModal.style.display = 'flex'; populateReminderTokenGrid({ grimoireState });
}
...
export function openCustomReminderEditModal({ grimoireState, playerIndex, reminderIndex, existingText = '' }) {
  ...
  const modalTitle = document.getElementById('custom-reminder-modal-title'); if (!customReminderEditModal || !customReminderTextInput || !saveCustomReminderBtn) return;
  grimoireState.editingCustomReminder = { playerIndex, reminderIndex }; customReminderTextInput.textContent = existingText; customReminderEditModal.style.display = 'flex';
  customReminderTextInput.focus();
  const handleKeyDown = (event) => {
    if (event.key === 'Escape') {
      event.stopPropagation(); customReminderEditModal.style.display = 'none'; grimoireState.editingCustomReminder = null;
  ...
  if (modalTitle) { modalTitle.textContent = reminderIndex === -1 ? 'Add Custom Reminder' : 'Edit Custom Reminder'; }
  const newSaveBtn = saveCustomReminderBtn.cloneNode(true); saveCustomReminderBtn.parentNode.replaceChild(newSaveBtn, saveCustomReminderBtn);
  newSaveBtn.addEventListener('click', withStateSave(() => {
    const newText = customReminderTextInput.textContent.trim();
    if (!newText) {
      customReminderEditModal.style.display = 'none'; grimoireState.editingCustomReminder = null; customReminderTextInput.removeEventListener('keydown', handleKeyDown); return;
    }
    if (grimoireState.editingCustomReminder) {
      ...
      if (rIdx === -1) {
        ...
        if (grimoireState.players[pIdx] && grimoireState.players[pIdx].reminders[rIdx]) { grimoireState.players[pIdx].reminders[rIdx].label = newText; }
      }
      updateGrimoire({ grimoireState });
    }
    customReminderEditModal.style.display = 'none'; grimoireState.editingCustomReminder = null; customReminderTextInput.removeEventListener('keydown', handleKeyDown);
  }));
} export function getVisibleRemindersCount({ grimoireState, playerIndex }) {
  const player = grimoireState.players[playerIndex]; if (!player || !player.reminders) return 0; let count = 0;
  player.reminders.forEach(reminder => {
    if (isReminderVisible(grimoireState, reminder.reminderId)) { count++; }
  }); return count;
}
function applyFontSize(element, text) {
  ...
  } else if (textLength > 20) { element.style.fontSize = 'clamp(8px, calc(var(--player-token-size) * 0.07), 12px)'; }
}
export function createReminderElement({
  ...
  let element;
  if (type === 'icon') {
    element = document.createElement('div'); element.className = `icon-reminder ${className}`.trim(); element.style.transform = `translate(-50%, -50%) rotate(${rotation}deg)`;
    if (isCustomIcon && label) {
      renderTokenElement({
        ...
        showLabel: false
      }); const textSpan = document.createElement('span'); textSpan.className = 'icon-reminder-content'; textSpan.textContent = label; applyFontSize(textSpan, label);
    ...
    element = document.createElement('div'); element.className = `text-reminder ${className}`.trim(); element.style.transform = 'translate(-50%, -50%)';
    const textSpan = document.createElement('span'); textSpan.className = 'text-reminder-content'; textSpan.textContent = label || ''; applyFontSize(textSpan, label || '');
    element.appendChild(textSpan);
  } else if (type === 'token') {
    element = document.createElement('div'); element.className = `token-reminder ${className}`.trim(); if (label) element.textContent = label;
    if (radiusFactor !== undefined) dataset.reminderRadius = String(radiusFactor); if (angleOffset !== undefined) dataset.reminderAngleOffset = String(angleOffset);
  }
  if (testId) element.setAttribute('data-testid', testId); if (title) element.title = title; if (ariaLabel) element.setAttribute('aria-label', ariaLabel);
...
export function renderRemindersForPlayer({ li, grimoireState, playerIndex }) {
  const remindersDiv = li.querySelector('.reminders'); if (!remindersDiv) return 0; remindersDiv.innerHTML = ''; let visibleRemindersCount = 0;
  const player = grimoireState.players[playerIndex]; if (!player || !Array.isArray(player.reminders)) return 0;
  player.reminders.forEach((reminder, idx) => {
    if (!isReminderVisible(grimoireState, reminder.reminderId)) {
      return; // Skip this reminder
    }
    visibleRemindersCount++; const isCustom = reminder.id === 'custom-note' || reminder.type === 'text';
    const onTap = (e, targetElement) => {
      const parentLi = li; const isCollapsed = !!(parentLi && parentLi.dataset.expanded !== '1'); const element = targetElement || e.currentTarget;
      if (isCollapsed) {
        e.stopPropagation();
        try { e.preventDefault(); } catch (_) { }
        parentLi.dataset.expanded = '1'; parentLi.dataset.actionSuppressUntil = String(Date.now() + CLICK_EXPAND_SUPPRESS_MS); positionRadialStack(parentLi, visibleRemindersCount);
        if (element && element.dataset) { element.dataset.ignoreNextSyntheticClick = 'true'; }
      } else if (isCustom) {
        e.stopPropagation();
        try { e.preventDefault(); } catch (_) { }
        if (parentLi) {
          const suppressUntil = parseInt(parentLi.dataset.actionSuppressUntil || '0', 10); const suppressForTouch = !!grimoireState.touchOccurred;
    ...
    const onLongPress = (e, x, y) => { showReminderContextMenu({ grimoireState, x, y, playerIndex, reminderIndex: idx }); };
