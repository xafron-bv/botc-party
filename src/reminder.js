...
export async function populateReminderTokenGrid({ grimoireState }) {
  ...
  if (reminderTokenGrid._delegatedSelectionHandler) {
    try {
      reminderTokenGrid.removeEventListener(
        'click',
        reminderTokenGrid._delegatedSelectionHandler,
        true
      );
    } catch (_) {}
  ...
  const delegatedSelectionHandler = withStateSave((e) => {
    ...
    try {
      ...
      /* ensure modal still closes even if add fails */
    }
    try {
      reminderTokenModal.style.display = 'none';
    } catch (_) {}
  ...
  try {
    ...
    const genericTokens = [
      {
        id: 'townsfolk-townsfolk',
        image: './assets/reminders/good-D9wGdnv9.webp',
        label: 'Townsfolk'
      },
      { id: 'wrong-wrong', image: './assets/reminders/evil-CDY3e2Qm.webp', label: 'Wrong' },
      {
        id: 'drunk-isthedrunk',
        image: './assets/reminders/drunk_g--QNmv0ZY.webp',
        label: 'Is The Drunk'
      },
      ...
      {
        id: 'virgin-noability',
        image: './assets/reminders/virgin_g-DfRSMLSj.webp',
        label: 'No Ability'
      }
    ];
    ...
    if (isPlayerMode) {
      try {
        Object.values(grimoireState.allRoles || {}).forEach((role) => {
          const imagePath = role.image || `/build/img/icons/${role.team}/${role.id}.webp`;
          const roleImage = resolveAssetPath(imagePath);
          playerModeCharacterTokens.push({
    ...
    const filtered = reminderTokens.filter((t) => {
      const combined =
        `${(t.label || '').toLowerCase()} ${(t.characterName || '').toLowerCase()}`.trim();
...
export function openReminderTokenModal({ grimoireState, playerIndex }) {
  ...
  if (reminderTokenModalPlayerName)
    reminderTokenModalPlayerName.textContent = grimoireState.players[playerIndex].name;
...
export function openTextReminderModal({
  grimoireState,
  playerIndex,
  reminderIndex = -1,
  existingText = ''
...
export function openCustomReminderEditModal({
  grimoireState,
  playerIndex,
  reminderIndex,
  existingText = ''
  ...
  if (onClick || onLongPress) {
    setupInteractiveElement({
      ...
      setTouchOccurred: (val) => {
        if (grimoireState) grimoireState.touchOccurred = val;
      },
