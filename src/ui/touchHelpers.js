...
function isPlayerOverlapping({ listItem }) {
  ...
  for (let i = 0; i < allPlayers.length; i++) {
    ...
    if (overlap) {
      const zIndex1 =
        parseInt(listItem.style.zIndex || window.getComputedStyle(listItem).zIndex, 10) || 0;
      const zIndex2 =
        parseInt(otherPlayer.style.zIndex || window.getComputedStyle(otherPlayer).zIndex, 10) || 0;
      if (zIndex2 >= zIndex1) {
        return true;
      }
    }
