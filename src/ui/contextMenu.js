  ...
  const buttons = [
    ...
    },
    {
      ...
      onClick: withStateSave(() => {
        ...
        hideReminderContextMenu({ grimoireState });
        if (playerIndex < 0 || reminderIndex < 0) return;
        if (!grimoireState.players[playerIndex] || !grimoireState.players[playerIndex].reminders)
          return;
...
export function ensurePlayerContextMenu({ grimoireState }) {
  ...
  const addPlayerAt = (offset) =>
    withStateSave(() => {
      ...
      grimoireState.players.splice(
        idx + offset,
        0,
        createEmptyPlayer(`Player ${grimoireState.players.length + 1}`)
      );
