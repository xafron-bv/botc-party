...
export function repositionPlayers({ grimoireState }) {
  ...
  const parentRect = circle.parentElement
    ...
    if (playerNameEl) {
      ...
      if (isTargetArc) {
        ...
        try {
          ...
          const tokenSize = tokenEl
            ? tokenEl.offsetWidth
            : parseFloat(getComputedStyle(listItem).getPropertyValue('--player-token-size')) || 64;
    ...
    if (players[i] && players[i].reminders) {
      players[i].reminders.forEach((reminder) => {
        if (
          ...
          !grimoireState.dayNightTracking.enabled ||
  ...
  settleAnimations(circle, 320, true);
}
