export function initInAppTour() {
  const startButton = document.getElementById('start-tour'); const sidebar = document.getElementById('sidebar'); const sidebarToggleBtn = document.getElementById('sidebar-toggle');
  if (!startButton || !sidebar || !sidebarToggleBtn) return; const backdrop = document.createElement('div'); backdrop.className = 'tour-backdrop';
  const highlight = document.createElement('div'); highlight.className = 'tour-highlight'; const pop = document.createElement('div'); pop.className = 'tour-popover';
  pop.setAttribute('role', 'dialog'); pop.setAttribute('aria-live', 'polite'); document.body.appendChild(backdrop); document.body.appendChild(highlight);
  document.body.appendChild(pop); const isSidebarCollapsed = () => document.body.classList.contains('sidebar-collapsed');
  function setSidebarCollapsed(collapsed) {
    const classCollapsed = isSidebarCollapsed();
    if (collapsed && !classCollapsed) {
      const closeBtn = document.getElementById('sidebar-close'); if (closeBtn) closeBtn.click(); else document.body.classList.add('sidebar-collapsed');
    } else if (!collapsed && classCollapsed) { sidebarToggleBtn.click(); }
  }
  function ensureVisibilityForStep(step) { if (step.requiresSidebarOpen) setSidebarCollapsed(false); else if (step.requiresSidebarClosed) setSidebarCollapsed(true); }
  function waitForAnimationsToFinish(element, fallbackMs = 400) {
    return new Promise((resolve) => {
      let resolved = false;
      function finish() {
        if (resolved) return; resolved = true;
        [document.body, sidebar, element].filter(Boolean).forEach((node) => {
          try {
      ...
      try {
        const nodes = [document.body, sidebar, element].filter(Boolean); const animations = [];
        for (let i = 0; i < nodes.length; i += 1) {
          const node = nodes[i];
          if (node && typeof node.getAnimations === 'function') {
            const arr = node.getAnimations({ subtree: true }); for (let j = 0; j < arr.length; j += 1) animations.push(arr[j]);
          }
        }
        if (animations.length > 0) {
          const timeoutId = setTimeout(finish, Math.max(250, fallbackMs));
          Promise.all(animations.map((a) => a.finished.catch(() => { }))).then(() => { clearTimeout(timeoutId); finish(); });
        } else { setTimeout(finish, Math.max(250, fallbackMs)); }
      } catch (_) { setTimeout(finish, Math.max(250, fallbackMs)); }
    });
  }
  function positionPopoverNear(rect) {
    const margin = 12; const popRect = pop.getBoundingClientRect(); let top = rect.bottom + margin; let left = rect.left;
    if (left + popRect.width > window.innerWidth - 8) left = Math.max(8, window.innerWidth - popRect.width - 8);
    if (top + popRect.height > window.innerHeight - 8) top = Math.max(8, rect.top - popRect.height - margin); pop.style.top = `${Math.max(8, top)}px`;
    pop.style.left = `${Math.max(8, left)}px`;
  }
  function showHighlight(rect) {
    const radius = 12; highlight.style.display = 'block'; highlight.style.left = `${rect.left - 8}px`; highlight.style.top = `${rect.top - 8}px`;
    highlight.style.width = `${rect.width + 16}px`; highlight.style.height = `${rect.height + 16}px`; highlight.style.borderRadius = `${radius}px`;
  }
  function hideUI() { backdrop.style.display = 'none'; highlight.style.display = 'none'; pop.style.display = 'none'; }
  const steps = [
    ...
    { id: 'bluff-tokens', title: 'Bluff Tokens', body: 'These are three bluff tokens. These are characters not in play that help the evil team to win. As a player, you can give these to other players in a three-for-three claim. Click a bluff token to assign a character to it.', target: () => document.querySelector('#bluff-tokens-container .bluff-token') || document.getElementById('bluff-tokens-container'), requiresSidebarClosed: true },
  ...
  function renderStep() {
    ...
    const doRender = () => {
      const rect = targetEl ? targetEl.getBoundingClientRect() : { left: 16, top: 16, width: 300, height: 60, right: 316, bottom: 76 }; backdrop.style.display = 'block';
      showHighlight(rect); pop.innerHTML = ''; const title = document.createElement('div'); title.className = 'title'; title.textContent = steps[idx].title;
      ...
      pop.appendChild(title); pop.appendChild(body); pop.appendChild(actions); pop.appendChild(progress); pop.style.display = 'block'; positionPopoverNear(rect);
    };
    if (targetEl && sidebar.contains(targetEl)) {
      try { targetEl.scrollIntoView({ block: 'center', inline: 'nearest' }); } catch (_) { }
    }
    waitForAnimationsToFinish(targetEl, 400).then(doRender);
  }
  function endTour() {
    hideUI(); window.removeEventListener('resize', handleResize, true); window.removeEventListener('orientationchange', handleResize, true);
    document.removeEventListener('keydown', handleKey, true);
  }
  function handleResize() {
    if (pop.style.display !== 'block') return; const targetEl = steps[idx].target && steps[idx].target(); if (!targetEl) return; const rect = targetEl.getBoundingClientRect();
    showHighlight(rect); positionPopoverNear(rect);
  }
  ...
  startButton.addEventListener('click', () => {
    idx = 0; renderStep(); window.addEventListener('resize', handleResize, true); window.addEventListener('orientationchange', handleResize, true);
    document.addEventListener('keydown', handleKey, true); backdrop.onclick = () => { if (idx < steps.length - 1) { idx += 1; renderStep(); } else { endTour(); } };
