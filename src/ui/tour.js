...
export function initInAppTour() {
  ...
  const steps = [
    ...
    { id: 'player-management', title: 'Add/Remove Players', body: () => { const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0; return isTouchDevice ? 'Long-press on any player to add or remove players during the game.' : 'Right-click on any player to add players before/after or remove them during the game.'; }, target: () => document.querySelector('#player-circle li .player-token') || document.getElementById('player-circle'), requiresSidebarClosed: true },
