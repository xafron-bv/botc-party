...
const storageKey = 'characterPanelOpen';
let setOpen = () => {};
...
export function initCharacterPanel({
  ...
  const syncSidebarToggle = () => {
    ...
    const hidden = viewport.matches && document.body.classList.contains('character-panel-open');
    sidebarToggle.style.display = hidden
      ? 'none'
      : document.body.classList.contains('sidebar-collapsed')
        ? 'inline-block'
        : 'none';
    sidebarToggle.style.visibility = hidden ? 'hidden' : '';
    sidebarToggle.style.pointerEvents = hidden ? 'none' : '';
  };
  const applyState = (open) => {
    panel.setAttribute('aria-hidden', String(!open));
    document.body.classList.toggle('character-panel-open', open);
    toggle.setAttribute('aria-pressed', String(open));
    toggle.textContent = 'Script';
    try {
  ...
  try {
    initiallyOpen = localStorage.getItem(storageKey) === '1';
  } catch (_) {}
  ...
  closeButtons.filter(Boolean).forEach((button) =>
    setupInteractiveElement({
      element: button,
  ...
  document.addEventListener('click', ({ target }) => {
    ...
    if (!panel.contains(target) && !toggle.contains(target) && !sidebar?.contains(target))
      applyState(false);
  });
  document.addEventListener('keydown', ({ key }) => {
    if (key === 'Escape' && document.body.classList.contains('character-panel-open'))
      applyState(false);
  });
  viewport.addEventListener('change', syncSidebarToggle);
