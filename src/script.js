...
const HISTORY_EXPORT_MESSAGE =
  'This appears to be a history export file. Please use the "Import History" button in the History Management section to import it.';
const isHistoryExport = (value) =>
  value &&
  typeof value === 'object' &&
  !Array.isArray(value) &&
  'version' in value &&
  'scriptHistory' in value &&
  'grimoireHistory' in value;
function statusWriter() {
  const element = byId('load-status');
  return (message, className = 'status') => {
    if (element) Object.assign(element, { textContent: message, className });
  };
}
function rejectHistoryExport(
  data,
  setStatus,
  shortMessage = 'This looks like a history export. Use Import History instead.'
) {
  if (!isHistoryExport(data)) return false;
  setStatus(shortMessage, 'error');
  alert(HISTORY_EXPORT_MESSAGE);
  return true;
}
function appendRole(characterSheet, role, simple = false) {
  const element = createElement('div', 'role');
  const image = role.image ? resolveAssetPath(role.image) : TOKEN_IMAGE;
  ...
  if (!simple) element.addEventListener('click', () => element.classList.toggle('show-ability'));
  characterSheet.appendChild(element);
}
function appendHeader(characterSheet, text, className) {
  characterSheet.appendChild(createElement('h3', className, text));
}
export async function displayScript({ data, grimoireState }) {
  const characterSheet = byId('character-sheet');
  console.log('Displaying script with', data.length, 'characters');
  characterSheet.innerHTML = '';
  let storedPanelState;
  try {
    storedPanelState = localStorage.getItem('characterPanelOpen');
  } catch (_) {}
  const autoOpen =
    !window.matchMedia('(max-width: 900px)').matches &&
    (storedPanelState === null ||
      (new URLSearchParams(window.location.search).has('test') && storedPanelState !== '0'));
  if (autoOpen) openCharacterPanel();
  const metaEntry = Array.isArray(data)
    ? data.find((x) => x && typeof x === 'object' && x.id === '_meta')
    : null;
  const scriptTitle = metaEntry?.name || grimoireState.scriptMetaName || '';
  const scriptAuthor = metaEntry?.author || '';
  const bootleggerNotes = Array.isArray(metaEntry?.bootlegger)
    ? metaEntry.bootlegger.filter(Boolean)
    : [];
  if (scriptTitle || scriptAuthor || bootleggerNotes.length) {
    const metaBlock = createElement('div', 'script-meta');
    if (scriptTitle) {
      metaBlock.appendChild(createElement('div', 'script-meta__title', scriptTitle));
    }
    if (scriptAuthor) {
      metaBlock.appendChild(createElement('div', 'script-meta__author', `Author: ${scriptAuthor}`));
    }
    if (bootleggerNotes.length) {
      const bootleggerEl = createElement('div', 'script-meta__bootlegger');
      const list = createElement('ul', 'script-meta__bootlegger-list');
      bootleggerEl.appendChild(createElement('div', 'script-meta__bootlegger-title', 'Bootlegger'));
      bootleggerNotes.forEach((note) => list.appendChild(createElement('li', '', note)));
  ...
  if (grimoireState.nightOrderSort) {
    ...
    const nightOrderCharacterIds =
      (grimoireState.nightOrderData && grimoireState.nightOrderData[nightOrderKey]) || [];
    ...
    rolesToRender.forEach(({ role }) => appendRole(characterSheet, role));
    ...
    if (Object.keys(teamGroups).length > 0) {
      ...
      teamOrder.forEach((team) => {
        if (teamGroups[team]?.length) {
          const name =
            team === 'traveller' ? 'Travellers' : team.charAt(0).toUpperCase() + team.slice(1);
          appendHeader(characterSheet, name, `team-${team === 'traveller' ? 'travellers' : team}`);
          teamGroups[team].forEach((role) => appendRole(characterSheet, role));
        }
        if (team === 'demon')
          displayJinxes({ jinxData, grimoireState, characterSheet, displayRoles });
      });
      ...
      data.forEach((characterItem) => {
        if (typeof characterItem === 'string' && characterItem !== '_meta') {
          ...
          characterItem !== null &&
          characterItem.id &&
          characterItem.id !== '_meta'
        ) {
          appendRole(characterSheet, characterItem);
        }
      });
    }
  }
...
export function loadScriptFromDataJson({ editionId, grimoireState }) {
  const pendingLoad = (async () => {
    ...
    try {
      ...
      setStatus(`Failed to load ${editionId}: ${error.message}`, 'error');
    }
  })();
  grimoireState.scriptLoadPromise = pendingLoad;
  return pendingLoad.finally(() => {
    if (grimoireState.scriptLoadPromise === pendingLoad) {
      grimoireState.scriptLoadPromise = null;
    }
  });
}
...
export const processScriptData = withStateSave(
  async ({ data, addToHistory = false, grimoireState }) => {
    ...
    try {
      ...
      grimoireState.scriptMetaName = '';
    }
    if (!Array.isArray(data)) {
      throw new Error(
        `Unexpected script format: ${typeof data}. Expected an array of script entries`
      );
    }
    ...
    if (addToHistory) {
      const histName =
        grimoireState.scriptMetaName ||
        (Array.isArray(data) &&
          (data.find((x) => x && typeof x === 'object' && x.id === '_meta')?.name ||
            'Custom Script')) ||
        'Custom Script';
...
export async function loadScriptFile({ event, grimoireState }) {
  ...
  reader.onload = async (e) => {
    ...
    try {
      await processScriptData({ data: json, addToHistory: true, grimoireState });
      setStatus('Custom script loaded successfully!');
      try {
        event.target.value = '';
      } catch (_) {}
    } catch (error) {
      console.error('Error processing uploaded script:', error);
      setStatus(`Invalid script file: ${error.message}`, 'error');
    }
  };
...
function displayJinxes({ jinxData, grimoireState, characterSheet, displayRoles }) {
  ...
  const scriptCharacterIds = new Set(Object.values(roles).map((role) => role.id));
  const applicableJinxes = jinxData.flatMap((character) =>
    !scriptCharacterIds.has(character.id)
      ? []
      : (character.jinx || [])
        .filter((jinx) => scriptCharacterIds.has(jinx.id))
  ...
  if (applicableJinxes.length > 0) {
    ...
    applicableJinxes.forEach((jinx) => {
      const jinxEl = createElement('div', 'jinx-entry');
      ...
      `;
      jinxEl.addEventListener('click', () => jinxEl.classList.toggle('show-jinx-reason'));
