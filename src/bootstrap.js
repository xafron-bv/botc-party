...
export function trackPageLoad() {
  const overlay = byId('app-loading');
  window.__overlayInitialVisible = Boolean(overlay);
  window.__overlayInitialMessage = byId('app-loading-message')?.textContent || '';
  let appReady = false;
  let windowReady = document.readyState === 'complete';
  let removed = false;
  const complete = () => {
    if (!appReady || !windowReady || removed) return;
    removed = true;
...
export async function showVersion() {
  const element = byId('app-version-value');
  if (!element) return;
  try {
    const response = await fetch('./version.json', { cache: 'no-store' });
    const data = response.ok ? await response.json() : null;
    element.textContent = data?.uiVersion || '?';
  } catch (_) {
    element.textContent = '?';
  }
}
