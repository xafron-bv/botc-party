...
export function initPlayerSetup({ grimoireState, collapseSidebar }) {
  const [
    ...
    playerSetupPanel,
    closePlayerSetupBtn,
    shuffleCharactersBtn,
    playerSetupCharacterList,
    bagCountWarning,
    numberPickerOverlay,
    numberPickerGrid,
    closeNumberPickerBtn,
    selectionPickerTitle,
    selectionPickerInstructions,
    selectionRevealBtn,
    playerRevealModal,
    closePlayerRevealModalBtn,
    confirmPlayerRevealBtn,
    revealCharacterTokenEl,
    revealHandoffLabelEl,
    revealHandoffNameEl,
    includeTravellersCheckbox,
    playerSetupCountsContainer
  ] = byIds(
    'open-player-setup',
    'player-setup-panel',
    'close-player-setup',
    'bag-shuffle',
    'player-setup-character-list',
    'bag-count-warning',
    'number-picker-overlay',
    'number-picker-grid',
    'close-number-picker',
    'selection-picker-title',
    'selection-picker-instructions',
    'selection-reveal-btn',
    'player-reveal-modal',
    'close-player-reveal-modal',
    'confirm-player-reveal',
    'reveal-character-token',
    'reveal-handoff-label',
    'reveal-handoff-name',
    'include-travellers-in-bag',
    'player-setup-counts'
  );
  const defaultBagWarningText = bagCountWarning
    ? bagCountWarning.textContent
    : 'Warning: Selected bag does not match player count configuration.';
  ...
  ['townsfolk', 'outsiders', 'minions', 'demons', 'travellers'].forEach((teamKey) => {
    const root = playerSetupCountsContainer
      ? playerSetupCountsContainer.querySelector(`[data-team="${teamKey}"]`)
      : null;
  ...
  function maybeReopenPanel() {
    ...
    if (grimoireState.playerSetup && grimoireState.playerSetup._reopenOnPickerClose) {
      ...
      try {
        playerSetupPanel.scrollIntoView({ block: 'center' });
      } catch (_) {}
  ...
  function updateSetupCountsDisplay({ teams, row }) {
    ...
    if (travellersElements && travellersElements.root) {
      ...
      if (travellersElements.selected)
        travellersElements.selected.textContent = String(travellerSelected);
    }
  ...
  function renderPlayerSetupList() {
    ...
    if (!allRoles.length) {
      const msg = createElement('div', '', 'Choose a script first');
    ...
    teamsOrder.forEach((team, idx) => {
      ...
      const header = createElement('div', 'team-header', team.label);
      playerSetupCharacterList.appendChild(header);
      const grid = document.createElement('div');
      grid.className = 'team-grid';
      orderedRoles.forEach((role) => {
        const isBagDisabled =
          Array.isArray(role.special) && role.special.some((s) => s && s.name === 'bag-disabled');
        ...
        const bagKey = isTraveller ? 'travellerBag' : 'bag';
        const getBag = () =>
          grimoireState.playerSetup[bagKey] || (grimoireState.playerSetup[bagKey] = []);
        const setRoleCount = (count) => {
          const bag = getBag();
          bag.splice(
            0,
            bag.length,
            ...bag.filter((id) => id !== role.id),
            ...Array(count).fill(role.id)
          );
        };
        checkbox.checked = getBag().includes(role.id) && !isBagDisabled;
        ...
        const toggle = withStateSave(() => {
          ...
          if (checkbox.checked) {
            setRoleCount(grimoireState.playerSetup.bagCounts?.[role.id] || 1);
      ...
      if (idx < teamsOrder.length - 1) {
        const sep = createElement('div', 'team-separator');
  ...
  const fillBagWithStandardSetup = withStateSave(() => {
    ...
    if (totalPlayers === 0) {
      if (bagCountWarning) {
        ...
        bagCountWarning.classList.add('error');
        try {
          bagCountWarning.scrollIntoView({ block: 'nearest' });
        } catch (_) {}
      }
      return;
    }
    ...
    if (!row) {
      if (bagCountWarning) {
        const travellerSuffix =
          travellerCount > 0
            ? ` after excluding ${travellerCount} traveller${travellerCount === 1 ? '' : 's'}`
            : '';
        bagCountWarning.style.display = 'block';
    ...
    Object.values(grimoireState.allRoles || {}).forEach((role) => {
      if (
        role &&
        Array.isArray(role.special) &&
        role.special.some((s) => s && s.name === 'bag-disabled')
      )
        return;
  ...
  const markSelectionCompleteIfDone = () => {
    try {
      ...
      clearNextPlayerHighlight();
      const openSetupBtn = byId('open-player-setup');
      if (openSetupBtn) {
  ...
  const openRevealModalForRole = ({ forIdx, role }) => {
    try {
      ...
      const handoffName =
        nextIdx === null
          ? 'Storyteller'
          : (grimoireState.players[nextIdx] && grimoireState.players[nextIdx].name) ||
            `Player ${nextIdx + 1}`;
  ...
  function openNumberPicker(forPlayerIndex) {
    ...
    const assignments = Array.isArray(grimoireState.playerSetup.assignments)
      ...
      : [];
    const existingPlayer = Array.isArray(grimoireState.players)
      ? grimoireState.players[forPlayerIndex]
      : null;
    const hasNumberAssignment =
      assignments[forPlayerIndex] !== null && assignments[forPlayerIndex] !== undefined;
    const hasCharacter = !!(existingPlayer && existingPlayer.character);
    if (hasNumberAssignment || hasCharacter) return;
    const playerName =
      existingPlayer && existingPlayer.name ? existingPlayer.name : `Player ${forPlayerIndex + 1}`;
    if (selectionPickerTitle) selectionPickerTitle.textContent = playerName;
    if (selectionPickerInstructions)
      selectionPickerInstructions.textContent = `If you're not ${playerName}, do not tap Reveal.`;
    ...
    if (selectionRevealBtn) {
      ...
      if (canRevealFromBag) {
        ...
        selectionRevealBtn.textContent =
          travellerBag.length > 0 ? 'Choose a Traveller below' : 'No characters left';
    ...
    try {
      numberPickerOverlay.style.position = 'fixed';
      numberPickerOverlay.style.inset = '0';
      numberPickerOverlay.style.zIndex = '9999';
    } catch (_) {}
    if (!isNumberGridHandlerAttached) {
      isNumberGridHandlerAttached = true;
      numberPickerGrid.addEventListener(
        'click',
        withStateSave((e) => {
          const travellerToken =
            e.target && e.target.closest && e.target.closest('.traveller-token');
          if (travellerToken) {
            ...
            updateGrimoire({ grimoireState });
            const li = byId('player-circle')?.children[forIdx];
            if (li) renderSelectionOverlay({ li, state: 'traveller' });
            highlightNext(forIdx);
    ...
    if (selectionRevealBtn && !isRevealButtonHandlerAttached) {
      ...
      selectionRevealBtn.addEventListener(
        'click',
        withStateSave(() => {
          ...
          const assignments = Array.isArray(grimoireState.playerSetup.assignments)
            ...
            : [];
          const hasNumberAssignment =
            assignments[forIdx] !== null && assignments[forIdx] !== undefined;
          const existingPlayer = Array.isArray(grimoireState.players)
            ? grimoireState.players[forIdx]
            : null;
          const hasCharacter = !!(existingPlayer && existingPlayer.character);
          if (hasNumberAssignment || hasCharacter) return;
          const bag = grimoireState.playerSetup.bag || [];
          const used = new Set(
            (grimoireState.playerSetup.assignments || []).filter(
          ...
          const li = byId('player-circle')?.children[forIdx];
          if (li) renderSelectionOverlay({ li, state: 'selected' });
          highlightNext(forIdx);
          numberPickerOverlay.style.display = 'none';
          openRevealModalForRole({ forIdx, role });
          markSelectionCompleteIfDone();
  ...
  try {
    window.openNumberPickerForSelection = (idx) => openNumberPicker(idx);
  } catch (_) {}
    ...
    openPlayerSetupBtn.addEventListener('click', () => {
      ...
      try {
        if (!grimoireState.gameStarted) {
          const [playerCountInput, grimoireHistoryList] = byIds(
            'player-count',
      ...
      try {
        document.body.classList.add('player-setup-open');
      } catch (_) {}
    });
  }
  ...
  if (shuffleCharactersBtn)
    shuffleCharactersBtn.addEventListener('click', shuffleCharacterOrderWithinTeams);
  ...
  if (startSelectionBtn)
    startSelectionBtn.addEventListener(
      'click',
      withStateSave(() => {
        const totalPlayers = grimoireState.players.length;
        const travellerCount = countTravellersInPlay();
        ...
        if (totalPlayers === 0) {
          if (bagCountWarning) {
            bagCountWarning.textContent =
              'Error: No players in grimoire. Please add players first.';
            bagCountWarning.style.display = 'block';
            bagCountWarning.classList.add('error');
            try {
              bagCountWarning.scrollIntoView({ block: 'nearest' });
        ...
        const selectedCount =
          grimoireState.playerSetup && grimoireState.playerSetup.bag
            ? grimoireState.playerSetup.bag.length
            : 0;
        if (selectedCount !== effectivePlayers) {
          if (bagCountWarning) {
            const travellerLabel = travellerCount === 1 ? 'traveller' : 'travellers';
            const travellerSuffix =
              travellerCount > 0 ? ` (excluding ${travellerCount} ${travellerLabel})` : '';
        ...
        if (playerCircle) {
          Array.from(playerCircle.children).forEach((li, idx) => {
            ...
            const assignments = grimoireState.playerSetup.assignments || [];
            const state = selectionState({ grimoireState, assignments, player, index: idx });
            renderSelectionOverlay({ li, state, onPick: () => openNumberPicker(idx) });
          });
        }
        highlightNext(-1);
  ...
  if (closeNumberPickerBtn && numberPickerOverlay)
    closeNumberPickerBtn.addEventListener('click', () => {
      numberPickerOverlay.style.display = 'none';
      maybeReopenPanel();
    });
  if (playerSetupPanel) {
    playerSetupPanel.addEventListener('click', (e) => {
      if (e.target === playerSetupPanel) {
        playerSetupPanel.style.display = 'none';
        try {
          document.body.classList.remove('player-setup-open');
        } catch (_) {}
