...
export function saveAppState({ grimoireState }) {
  try {
    ...
    try {
      localStorage.setItem(INCLUDE_TRAVELLERS_KEY, grimoireState.includeTravellers ? '1' : '0');
    } catch (_) {}
    try {
      localStorage.setItem(
        MODE_STORAGE_KEY,
        grimoireState.mode === 'player' ? 'player' : 'storyteller'
      );
    } catch (_) {}
  } catch (_) {}
}
export async function loadAppState({ grimoireState, grimoireHistoryList }) {
  try {
    ...
    if (saved && Array.isArray(saved.scriptData) && saved.scriptData.length) {
      ...
      if (saved.scriptMetaName) {
        grimoireState.scriptMetaName = String(saved.scriptMetaName);
      }
      if (saved.includeTravellers) {
        grimoireState.includeTravellers = saved.includeTravellers;
      }
    }
