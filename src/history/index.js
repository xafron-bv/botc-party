...
export function loadHistories() {
  try {
    const sRaw = localStorage.getItem('botcScriptHistoryV1'); if (sRaw) history.scriptHistory = JSON.parse(sRaw) || [];
  } catch (_) { history.scriptHistory = []; }
  try {
    const gRaw = localStorage.getItem('botcGrimoireHistoryV1'); if (gRaw) history.grimoireHistory = JSON.parse(gRaw) || [];
