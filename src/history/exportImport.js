...
export async function importUserData(file) {
  try {
    ...
    if (
      !data.scriptHistory ||
      !data.grimoireHistory ||
      !Array.isArray(data.scriptHistory) ||
      !Array.isArray(data.grimoireHistory)
    ) {
      throw new Error(
        'Invalid user data format: missing or invalid scriptHistory/grimoireHistory arrays'
      );
    }
...
export function initExportImport({ grimoireState, grimoireHistoryList } = {}) {
  ...
  if (exportBtn) {
    exportBtn.addEventListener('click', () => {
      const exportType = exportTypeSelect
        ? String(exportTypeSelect.value || 'full-data')
        : 'full-data';
