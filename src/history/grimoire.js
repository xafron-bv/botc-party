...
function isGrimoireStateEqual(state1, state2) {
  ...
  for (let i = 0; i < state1.players.length; i++) {
    ...
    for (let j = 0; j < r1.length; j++) {
      if (
        r1[j].type !== r2[j].type ||
        r1[j].token !== r2[j].token ||
        r1[j].text !== r2[j].text ||
        r1[j].label !== r2[j].label
      )
        return false;
    }
  }
...
export function snapshotCurrentGrimoire({
  players,
  scriptMetaName,
  scriptData,
  grimoireHistoryList,
    ...
    const currentState = {
      ...
      scriptName:
        ...
        (Array.isArray(scriptData) &&
          (scriptData.find((x) => x && typeof x === 'object' && x.id === '_meta')?.name || '')) ||
        '',
      scriptData,
      winner: winner || null
    };
...
export async function handleGrimoireHistoryClick({ e, grimoireHistoryList, grimoireState }) {
  ...
  if (grimoireState.gameStarted && !grimoireState.winner) {
    const ok = window.confirm(
      'A game is in progress. Loading history will reset the current game. Continue?'
    );
    if (!ok) return;
  }
  if (
    !isGrimoireStateEqual(currentState, entryState) &&
    window.grimoireState &&
    window.grimoireState.gameStarted
  ) {
    try {
      if (
...
export function handleGrimoireHistoryOnClear() {
  document
    .querySelectorAll('#grimoire-history-list li.pressed')
...
export const restoreGrimoireFromEntry = withStateSave(
  async ({ entry, grimoireState, grimoireHistoryList }) => {
    if (!entry) return;
    try {
      ...
      try {
        ...
        if (openSetupBtn)
          openSetupBtn.style.display = grimoireState.mode === 'player' ? 'none' : '';
...
export function addGrimoireHistoryListListeners({ grimoireHistoryList, grimoireState }) {
  ...
  grimoireHistoryList.addEventListener('click', async (e) =>
    handleGrimoireHistoryClick({ e, grimoireHistoryList, grimoireState })
  );
  grimoireHistoryList.addEventListener('keydown', (e) =>
    handleGrimoireHistoryOnKeyDown({ e, grimoireHistoryList })
  );
}
