...
function isGrimoireStateEqual(state1, state2) {
  if (!state1.players || !state2.players) return false; if (state1.players.length !== state2.players.length) return false;
  for (let i = 0; i < state1.players.length; i++) {
    const p1 = state1.players[i]; const p2 = state2.players[i]; if (p1.name !== p2.name || p1.character !== p2.character || p1.dead !== p2.dead) return false;
    const r1 = p1.reminders || []; const r2 = p2.reminders || []; if (r1.length !== r2.length) return false;
...
export function snapshotCurrentGrimoire({ players, scriptMetaName, scriptData, grimoireHistoryList, dayNightTracking, winner, gameStarted = true }) {
  try {
    ...
    const snapPlayers = JSON.parse(JSON.stringify(players)); const name = formatDateName(new Date());
    const entry = {
...
export async function handleGrimoireHistoryClick({ e, grimoireHistoryList, grimoireState }) {
  ...
  if (clickedDelete) {
    if (confirm('Delete this grimoire snapshot?')) {
      history.grimoireHistory = history.grimoireHistory.filter(x => x.id !== id); saveHistories(); renderGrimoireHistory({ grimoireHistoryList });
    }
    return;
  }
  if (clickedRename) {
...
export const restoreGrimoireFromEntry = withStateSave(async ({ entry, grimoireState, grimoireHistoryList }) => {
  ...
  try {
    ...
    if (entry.scriptData) {
      await processScriptData({ data: entry.scriptData, addToHistory: false, grimoireState }); grimoireState.scriptMetaName = entry.scriptName || grimoireState.scriptMetaName;
    }
    setupGrimoire({ grimoireState, grimoireHistoryList, count: (entry.players || []).length || 0 }); grimoireState.players = JSON.parse(JSON.stringify(entry.players || []));
    grimoireState.winner = entry.winner || null; grimoireState.gameStarted = !!entry.gameStarted;
    if (entry.dayNightTracking) { grimoireState.dayNightTracking = JSON.parse(JSON.stringify(entry.dayNightTracking)); initDayNightTracking(grimoireState); }
