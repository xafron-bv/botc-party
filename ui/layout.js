...
export function updateGrimoire(players, allRoles, isTouchDevice, options = {}) {
	const CLICK_EXPAND_SUPPRESS_MS = typeof options.CLICK_EXPAND_SUPPRESS_MS === 'number' ? options.CLICK_EXPAND_SUPPRESS_MS : 250;
	const showReminderContextMenu = options.showReminderContextMenu || null;
	const requestUpdate = typeof options.updateGrimoire === 'function' ? options.updateGrimoire : null;
	const persistState = typeof options.saveAppState === 'function' ? options.saveAppState : null;
	...
	listItems.forEach((li, i) => {
		...
		player.reminders.forEach((reminder, idx) => {
			if (reminder.type === 'icon') {
				...
				// Expand parent if collapsed on direct icon click
				...
				if (isTouchDevice && typeof showReminderContextMenu === 'function') {
