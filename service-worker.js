...
const _cacheNameReady = fetch(VERSION_URL)
  ...
  .catch(() =>
    caches.keys().then((names) => {
      const latest = names
        .filter((n) => n.startsWith('botc-party-v'))
        .sort()
        .pop();
        ...
        const fetchAndCache = async (filePath) => {
          ...
          if (newHashMap && oldHashMap && oldCache) {
            ...
            if (newHash && oldHash && newHash === oldHash) {
              ...
              if (oldResponse) {
                await cache.put(
                  new Request(absoluteUrl, { mode: 'same-origin' }),
                  oldResponse.clone()
                );
          ...
          if (!response || !response.ok) {
            throw new Error(
              `HTTP ${response ? response.status : '0'} while caching ${absoluteUrl}`
            );
          }
    ...
    event.respondWith(
      (async () => {
        ...
        try {
          ...
          /* cache lookup failed -- fall through to offline message */
        }
        return new Response(
          'App is offline and not yet cached. Please connect to the internet and reload.',
          {
            status: 503,
...
async function prefetchSpecificAssets(files) {
  ...
  const workers = Array.from(
    { length: Math.min(CONCURRENCY, Math.max(1, toFetch.length)) },
    worker
  );
...
async function findCachedIndexHtml() {
  ...
  const global =
    (await caches.match(INDEX_HTML_CACHE_KEY, { ignoreSearch: true })) ||
  ...
  for (const name of names
    ...
    const match =
      (await cache.match(INDEX_HTML_CACHE_KEY, { ignoreSearch: true })) ||
