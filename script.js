...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const bootstrap = async () => {
    ...
    try { const storedMode = localStorage.getItem(MODE_STORAGE_KEY); grimoireState.mode = storedMode === 'storyteller' ? 'storyteller' : 'player'; } catch (_) {
      grimoireState.mode = 'player';
    }
    const updateGrimoireControlButtons = () => {
      const isPlayer = grimoireState.mode === 'player';
      if (revealToggleBtn) {
        const hidden = !!grimoireState.grimoireHidden; revealToggleBtn.style.display = isPlayer ? '' : 'none';
    ...
    const updateSnapshotToggleUI = () => {
      if (!grimoireSnapshotToggleBtn) return; const isStoryteller = grimoireState.mode === 'storyteller'; grimoireSnapshotToggleBtn.style.display = isStoryteller ? '' : 'none';
      const active = hasGrimoireSnapshot(grimoireState); grimoireSnapshotToggleBtn.setAttribute('aria-pressed', String(active));
    ...
    const applyModeUI = () => {
      ...
      const openPlayerSetupBtn2 = byId('open-player-setup'); if (openPlayerSetupBtn2) openPlayerSetupBtn2.style.display = isPlayer ? 'none' : '';
      if (openRulebookBtn) openRulebookBtn.style.display = isPlayer ? 'none' : ''; const _openStBtn = byId('open-storyteller-message');
      ...
      updateGrimoireControlButtons(); updateSnapshotToggleUI();
    };
    function applyGrimoireHiddenUI() { applyGrimoireHiddenState({ grimoireState }); updateGrimoireControlButtons(); }
    ...
    if (revealToggleBtn) {
      revealToggleBtn.addEventListener('click', () => { if (grimoireState.mode !== 'player') return; toggleGrimoireHidden({ grimoireState }); updateGrimoireControlButtons(); });
    }
    if (grimoireSnapshotToggleBtn) {
      grimoireSnapshotToggleBtn.addEventListener('click', () => {
        if (grimoireState.mode !== 'storyteller') return;
        if (hasGrimoireSnapshot(grimoireState)) { restoreGrimoireSnapshot({ grimoireState }); } else {
    ...
    if (modeStorytellerRadio && modePlayerRadio) {
      ...
      const onModeChange = (e) => {
        const val = e && e.target && e.target.value; const nextMode = (val === 'player') ? 'player' : 'storyteller'; if (nextMode === grimoireState.mode) return;
        ...
        grimoireState.gameStarted = false; grimoireState.mode = nextMode; applyModeUI();
        try { localStorage.setItem(MODE_STORAGE_KEY, grimoireState.mode); } catch (_) { }
        saveAppState({ grimoireState });
      }; modeStorytellerRadio.addEventListener('change', onModeChange); modePlayerRadio.addEventListener('change', onModeChange);
    }
    try {
    ...
    try {
      grimoireState.nightOrderSort = (localStorage.getItem('nightOrderSort') === '1'); grimoireState.nightPhase = localStorage.getItem('nightPhase') || 'first-night';
    } catch (_) { grimoireState.nightOrderSort = false; grimoireState.nightPhase = 'first-night'; }
    if (nightOrderSortCheckbox) {
      nightOrderSortCheckbox.checked = grimoireState.nightOrderSort;
      if (nightOrderControls) { nightOrderControls.classList.toggle('active', grimoireState.nightOrderSort); }
      if (nightPhaseToggleBtn) { nightPhaseToggleBtn.style.display = grimoireState.nightOrderSort ? '' : 'none'; }
      const nightPhaseContainer = document.querySelector('.night-phase-buttons');
      if (nightPhaseContainer) { nightPhaseContainer.style.display = grimoireState.nightOrderSort ? '' : 'none'; }
      nightOrderSortCheckbox.addEventListener('change', async () => {
        ...
        if (nightPhaseContainer2) { nightPhaseContainer2.style.display = grimoireState.nightOrderSort ? '' : 'none'; }
        if (grimoireState.scriptData) { await displayScript({ data: grimoireState.scriptData, grimoireState }); }
      });
    }
    if (firstNightBtn && otherNightsBtn) {
      firstNightBtn.checked = grimoireState.nightPhase === 'first-night'; otherNightsBtn.checked = grimoireState.nightPhase === 'other-nights';
      const handlePhaseChange = async (e) => {
        ...
        if (grimoireState.scriptData && grimoireState.nightOrderSort) { await displayScript({ data: grimoireState.scriptData, grimoireState }); }
      }; firstNightBtn.addEventListener('change', handlePhaseChange); otherNightsBtn.addEventListener('change', handlePhaseChange);
      if (nightPhaseToggleBtn) {
        nightPhaseToggleBtn.addEventListener('click', async () => {
          const newPhase = grimoireState.nightPhase === 'first-night' ? 'other-nights' : 'first-night'; grimoireState.nightPhase = newPhase;
          if (firstNightBtn) firstNightBtn.checked = newPhase === 'first-night'; if (otherNightsBtn) otherNightsBtn.checked = newPhase === 'other-nights';
    ...
    if (loadTbBtn) {
      loadTbBtn.addEventListener('click', async () => {
        grimoireState.scriptMetaName = 'Trouble Brewing'; renderSetupInfo({ grimoireState }); await loadScriptFromDataJson({ editionId: 'tb', grimoireState });
      });
    }
    if (loadBmrBtn) {
      loadBmrBtn.addEventListener('click', async () => {
        grimoireState.scriptMetaName = 'Bad Moon Rising'; renderSetupInfo({ grimoireState }); await loadScriptFromDataJson({ editionId: 'bmr', grimoireState });
      });
    }
    if (loadSavBtn) {
      loadSavBtn.addEventListener('click', async () => {
        grimoireState.scriptMetaName = 'Sects & Violets'; renderSetupInfo({ grimoireState }); await loadScriptFromDataJson({ editionId: 'snv', grimoireState });
      });
    }
    if (loadAllCharsBtn) {
      loadAllCharsBtn.addEventListener('click', () => {
        grimoireState.scriptMetaName = 'All Characters'; renderSetupInfo({ grimoireState }); loadAllCharacters({ grimoireState });
      });
    }
    scriptFileInput.addEventListener('change', (event) => loadScriptFile({ event, grimoireState }));
    if (loadScriptTextBtn && scriptTextInput) { loadScriptTextBtn.addEventListener('click', () => loadScriptFromText({ grimoireState, text: scriptTextInput.value })); }
    if (loadScriptUrlBtn && scriptUrlInput) { loadScriptUrlBtn.addEventListener('click', () => loadScriptFromUrl({ grimoireState, url: scriptUrlInput.value })); }
    const applySharedScriptFromUrl = async () => {
      const params = new URLSearchParams(window.location.search); const encodedScript = params.get('script'); const scriptUrlParam = params.get('scriptUrl');
      if (encodedScript) {
        const data = decodeSharedScriptParam(encodedScript);
        if (!data) {
          const loadStatus = byId('load-status');
          if (loadStatus) { loadStatus.textContent = 'Could not load shared script (invalid share link).'; loadStatus.className = 'error'; }
          return;
        }
        const loadStatus = byId('load-status');
        const updateStatus = (text, className = 'status') => {
          if (!loadStatus) return; loadStatus.textContent = text; loadStatus.className = className;
        }; updateStatus('Loading shared script...');
        try {
          await processScriptData({ data, addToHistory: true, grimoireState }); await displayScript({ data: grimoireState.scriptData, grimoireState });
          renderSetupInfo({ grimoireState }); saveAppState({ grimoireState }); if (typeof window.updateButtonStates === 'function') window.updateButtonStates();
          updateStatus('Shared script loaded');
        } catch (err) { console.error('Failed to load shared script', err); updateStatus('Could not load shared script', 'error'); }
        return;
      }
      if (scriptUrlParam) { await loadScriptFromUrl({ grimoireState, url: scriptUrlParam }); }
    }; applySharedScriptFromUrl().catch((err) => console.error('Failed to load shared script', err)); loadPlayerSetupTable({ grimoireState });
