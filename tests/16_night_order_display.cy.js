describe('Night Order Display', () => {
  ...
  describe('Night Order Numbers', () => {
    ...
    it('should show night order numbers when night slider is open during night phase', () => {
      ...
      cy.get('#player-circle li').eq(1).find('[data-testid="night-order-number"]').should('exist');

      // Check Poisoner (first night order 17)
      cy.get('#player-circle li').eq(2).find('[data-testid="night-order-number"]').should('exist');

      // Check Fortune Teller (first night order 37)
      cy.get('#player-circle li').eq(4).find('[data-testid="night-order-number"]').should('exist');

      // Soldier should not have a night order number (no night ability)
      cy.get('#player-circle li').eq(3).find('[data-testid="night-order-number"]').should('not.exist');
    ...
    it('should update night order numbers when moving to other nights', () => {
      ...
      cy.get('#player-circle li').eq(0).find('[data-testid="night-order-number"]').should('exist');
    ...
    it('should position night order numbers to avoid overlap with ability info icons in touch mode', () => {
      ...
      cy.get('#player-circle li').eq(0).find('[data-testid="night-order-number"]').then($order => {
    ...
    it('should show demon and minion reminder buttons for night setup info', () => {
      ...
      cy.get('#player-circle li').eq(1).within(() => {
      ...
      cy.get('#player-circle li').eq(2).within(() => {
        cy.get('[data-testid="night-reminder-demon"]').should('contain', 'D');
      });

      // Clicking demon bluff reminder should show bluff names
      cy.get('#player-circle li').eq(0).find('[data-testid="night-reminder-bluffs"]').click({ force: true });
      ...
      cy.get('#player-circle li').eq(0).find('[data-testid="night-reminder-minions"]').click({ force: true });
      cy.get('#storyteller-message-display').should('be.visible');
      ...
      cy.get('#player-circle li').eq(2).find('[data-testid="night-reminder-demon"]').click({ force: true });
      cy.get('#storyteller-message-display').should('be.visible');
  ...
  describe('Night Order with Different Scripts', () => {
    it('should handle characters from different scripts correctly', () => {
      ...
      cy.get('#player-circle li').eq(0).find('[data-testid="night-order-number"]').should('not.exist');
  ...
  describe('Night Order Calculation', () => {
    ...
    it('should correctly calculate sequential night order numbers', () => {
      ...
      cy.get('#player-circle li').eq(0).find('[data-testid="night-order-number"]').should('contain', '1');
      cy.get('#player-circle li').eq(1).find('[data-testid="night-order-number"]').should('contain', '2');
      cy.get('#player-circle li').eq(2).find('[data-testid="night-order-number"]').should('contain', '3');
      cy.get('#player-circle li').eq(3).find('[data-testid="night-order-number"]').should('contain', '4');
    });
  });
});
